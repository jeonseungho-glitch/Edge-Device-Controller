...
# [MOD] Steering second
L1_GO  = 0.18  # left level 1
L2_GO  = 0.30  # left level 2
R1_GO  = 0.20  # right
R2_GO  = 0.45 #

L1_RET = 0.12  # left return level 1
L2_RET = 0.20  # left 2
R1_RET = 0.11  # right 1
R2_RET = 0.18  # right 2
...
# =========================================================
# 카메라가 오토바이에 고정 + "숙여진 건 직선으로 하면 될 것 같아" 라고 했으니,
# pitch는 0으로 두고 시작 (필요하면 -3~-10도 정도로 조정)
CAMERA_HEIGHT_M    = 1.20    # 카메라 높이(m) (오토바이 기준 대략 1.0~1.5)
FOV_VERTICAL_DEG   = 60.0    # 수직 FOV(근사). 모르면 55~70 사이에서 보정
PITCH_DOWN_DEG     = 0.0     # 아래로 숙인 각도(도). 아래로 숙였으면 +값(예: 5~12)
...
def get_ret_time(side, level):
    """
    side/level 상태에서 센터로 돌아오기 위해 필요한 '복귀 시간(크기)' 테이블.
    실제 펄스 방향은 반대 방향으로 준다.
    """
    if side == "L" and level == 1:
        return L1_RET
    if side == "L" and level == 2:
...
def servo_pulse(spwm, side, seconds):
    ...
    # NOTE: 만약 좌/우가 실제 하드웨어와 반대면 아래 speed 부호만 swap 하면 됨.
...
def apply_steer_state_transition(spwm, prev_state, new_state):
    ...
    FINAL transition:
      - center->side : GO(same)
      - side->center : RET(opposite)
      - same side: up=GO diff(same), down=RET diff(opposite)
      - change side: RET(opposite of prev) then GO(new)
    """
...
def ipm_distance_from_y(y_pix, img_h):
    """
    y_pix: 픽셀 y (원본 이미지 좌표)
    img_h: 원본 이미지 높이
    return: forward ground distance (meters)
    """
    cy = img_h * 0.5
    fy = _compute_fy_from_fov(img_h, FOV_VERTICAL_DEG)

    theta = np.arctan((float(y_pix) - cy) / max(fy, 1e-6))          # optical axis 기준
    pitch = np.deg2rad(float(PITCH_DOWN_DEG))                       # 아래로 숙인 각도(+)
    alpha = theta + pitch                                           # 지면으로 향하는 각
...
def decide_control_state(comm_ok,
    ...
    # signal + stopline/crosswalk rule
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # apply servo state transition exactly like your code
            ...
            # ---- STATE INPUTS (🔥 depth 대신 IPM distance meters) ----
            ...
            for trk in tracks:
                ...
                # ✅ 대표점: bbox "밑변 중점" (요청 반영)
                scx = int(0.5 * (sx1 + sx2))
                scy = int(sy2)  # bottom y

                # ✅ IPM distance (meters) (매 프레임)
